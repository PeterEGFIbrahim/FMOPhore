...
if (args.directory or args.Prot_complex or args.PDB_ID or (args.protein_pdb_file and args.ligand_files)) and args.distance_cutoff and args.qm_calculation and args.timer and args.cpus:
    pass
else:
    parser.print_help()
    sys.exit(1)
        ...
        if not any(exclude_str in ligand_info for exclude_str in excluded_strings):
            ...
            if os.path.exists(ligand_dir):
                ...
                while any(not os.path.exists(file) for file in input_files):
                    time.sleep(20) 
                if args.qm_calculation == "DFTB":
                    command5 = os.path.join(".", "run_DFTB_personalized.sh")
                    run_command(command5)
                elif args.qm_calculation == "MP2": 
                    command5 = os.path.join(".", "run_MP2_personalized.sh")
                    run_command(command5)
                        ...
                        for log_file in log_files:
                            with open(log_file, 'r') as f:
                                ...
                                if error.strip() in file_contents:
                                    ...
                                    if args.qm_calculation == "DFTB":
                                        command5 = os.path.join(".", "run_DFTB.sh")
                                        run_command(command5)
                                    elif args.qm_calculation == "MP2": 
                                        command5 = os.path.join(".", "run_MP2.sh")
                                        run_command(command5)
                                    time.sleep(20)   
                                else:
                                    pass      
