...
class Pharmacophores:
    ...
    def run_ph4(self):
        ...
        non_empty_lines = [line for line in non_empty_lines if not line.startswith('**')] 
        non_empty_lines = [line for line in non_empty_lines if 'LIG' in line]
        non_empty_lines = [line for line in non_empty_lines if not line.startswith('Excluded molecules as ligands:')] 
        non_empty_lines = [line for line in non_empty_lines if not line.startswith('No interactions detected.')] 
        non_empty_lines = [line for line in non_empty_lines if line[16:17] != 'B']
        non_empty_lines = [line for line in non_empty_lines if line[16:17] != 'C']
        non_empty_lines = [line for line in non_empty_lines if line[16:17] != 'D']
        non_empty_lines = [line for line in non_empty_lines if line[16:17] != 'E']
        non_empty_lines = [line for line in non_empty_lines if 'DMS' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'DMS' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'BO3' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'GOL' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'AGOL' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'BGOL' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'ZN' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'EDO' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'SO4' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'ACE' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'NMA' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'NME' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'ACT' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'MES' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'OCY' not in line]
        non_empty_lines = [line for line in non_empty_lines if 'SMALLMOLECULE' not in line]
        modified_lines = []
