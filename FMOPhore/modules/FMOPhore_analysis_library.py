...
class LibAnalysis:
    ...
    def Lib_analyze(self):
            ...
            for subdir_chain in os.listdir(subdir_path):
                ...
                if os.path.isdir(analysis_dir):
                    for filename in os.listdir(analysis_dir):
                        if filename.endswith("Energies.txt"):
                            filepath = os.path.join(analysis_dir, filename)
                            match = re.match(r"^(.*?_FMO)(?:_lig_H_(frag\d+)_H_FE)?_Energies\.txt$", filename)
                            if match:
                                base = match.group(1).replace("_FMO", "")
                                frag_suffix = match.group(2)
                                fragment_name = f"{base}_{frag_suffix}" if frag_suffix else base
                            else:
                                # Fallback: extract up to _FMO manually
                                base_match = re.match(r"^(.*?)(_FMO.*)$", filename)
                                fragment_name = base_match.group(1) if base_match else filename.replace("_Energies.txt", "")
                            with open(filepath, 'r') as infile:
                                ...
                                if interaction_energy is not None:
                                    all_interaction_energies.append((fragment_name, interaction_energy))
        sorted_interaction_energies = sorted(all_interaction_energies, key=lambda x: x[1])
        output_dir = os.path.join(parent_dir, 'library_analysis')
        os.makedirs(output_dir, exist_ok=True)
        with open(os.path.join(output_dir, 'Interaction_energies.txt'), 'w') as outfile:
            outfile.write("Fragment   Interaction_energy\n")
            for fragment_name, energy in sorted_interaction_energies:
                outfile.write(f"{fragment_name}    {energy:.3f}\n")
        ###################################################################################################
        parent_dir = os.getcwd()
        ...
        parent_dir = os.getcwd()
        all_selected_data_list = []  # List to collect DataFrames
        for subdir in os.listdir(parent_dir):
            subdir_path = os.path.join(parent_dir, subdir)
            if not os.path.isdir(subdir_path):
                continue
                ...
                if os.path.isdir(analysis_dir):
                    ...
                    for file in os.listdir(analysis_dir):
                        if file == 'Ph4_report.csv':
                            ...
                            selected = selected.T
                            all_selected_data_list.append(selected)
                os.chdir(parent_dir)
        if all_selected_data_list:
            all_selected_data = pd.concat(all_selected_data_list)
        else:
        ...
        plt.xlabel('Binding Site Residues', fontsize=20)  
        plt.ylabel('PDB IDs', fontsize=20) 
        plt.title('Percentage of Interaction %', fontsize=20)  
        ...
        sns.set_style('darkgrid')
        plt.figure(figsize=(20, 10))
        if selected_data.empty or selected_data.isna().all().all():
            print("Warning: selected_data DataFrame is empty or all NaNs — skipping heatmap.")
            return
        ...
        for i, line in enumerate(lines):
            ...
            try:
                ...
                chain_id_BS = residue_id_BS[3:4]
                residue_number_BS = residue_id_BS[4:]
                residue_id_BS = residue_id_BS[:3]
                output_line_BS = "{:<6s}{:5d} {:>4s} {:<3s} {:1s}{:4d}    {:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}          {:>2s}{:>2d}\n".format(
                    "ATOM", i, atom_name_BS, residue_id_BS, chain_id_BS, int(residue_number_BS),
                    float(coordinates_BS[0]), float(coordinates_BS[1]), float(coordinates_BS[2]),
                    0.00, 0.00, atom_name_BS[0], 0
                )
                output_lines_BS.append(output_line_BS)
            except (ValueError, IndexError) as e:
                print(f"⚠️ Skipping line {i + 1} due to error: {e}")
                continue  
        ...
        for fragment in unique_fragments:
            ...
            ax1.scatter(subset['frequency'], subset['Energy'], color=color_map[fragment], s=100, edgecolor='black') 
            for i, row in subset.iterrows():
                texts.append(ax1.text(row['frequency'], row['Energy'], row['Formatted_Fragment'], fontsize=20)) 
        adjust_text(texts, ax=ax1, arrowprops=dict(arrowstyle='-', color='grey', lw=0.5))
        ...
        def process_fmophore(fmophore_folder):
            ...
            ph4_lig_file = os.path.join(fmophore_folder, 'Ph4_lig.pdb')  
        ...
        ###############################################################################
        ###############################################################################
        def merge_fe_score_files(fe_score_folder, output_file):
            fe_score_files = glob.glob(os.path.join(fe_score_folder, '*.txt'))
            df_list = []
            for file in fe_score_files:
            ...
            return merged_df
        ###############################################################################    
        def merge_fmo_phore_data(parent_folder, merged_fe_score_file):
            fe_score_df = pd.read_csv(merged_fe_score_file, sep='\t')
            if 'Matched_PDB' not in fe_score_df.columns:
                raise KeyError("Column 'Matched_PDB' not found in FE score file.")
            fe_score_df['Matched_PDB'] = fe_score_df['Matched_PDB'].str.replace('_H_FE.pdb', '', regex=False)
            fmophore_folders = glob.glob(os.path.join(parent_folder, '**/FMOPhore'), recursive=True)
            fmophore_data = []
            for folder in fmophore_folders:
                fe_scores_results_file = os.path.join(folder, 'FE_score', 'FE_scores_results_with_score.txt')
                if os.path.exists(fe_scores_results_file):
                    df_fescore = pd.read_csv(fe_scores_results_file, sep='\t')
                    if 'PDB-ID' in df_fescore.columns:
                        df_fescore['PDB-ID'] = df_fescore['PDB-ID'].str.replace('_H_FE.pdb', '', regex=False)
                        fmophore_data.append(df_fescore)
                    else:
                        print("Warning: 'PDB-ID' column not found in", fe_scores_results_file)
            if fmophore_data:
                fmophore_df = pd.concat(fmophore_data, ignore_index=True)
                final_df = fe_score_df.merge(fmophore_df, left_on='Matched_PDB', right_on='PDB-ID', how='left')
                final_df.to_csv(merged_fe_score_file, sep='\t', index=False)
                return final_df
            else:
                print("No FMOPhore data found.")
                return fe_score_df
        ###############################################################################        
        def generate_fragment_barcharts(merged_fe_score_file, output_dir, pdb_folder):
            df = pd.read_csv(merged_fe_score_file, sep='\t')
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            unique_fragments = df['Fragment'].unique()
            cmap = get_cmap("Set2")
            for fragment in unique_fragments:
                fragment_df = df[df['Fragment'] == fragment]
                fragment_dir = os.path.join(output_dir, fragment)
                if not os.path.exists(fragment_dir):
                    os.makedirs(fragment_dir)
                fe_score_file = os.path.join(fragment_dir, "FE_score_values.txt")
                with open(fe_score_file, "w") as f:
                    sorted_scores = fragment_df[['PDB-ID', 'FE-score']].sort_values(by='FE-score', ascending=False)
                    for _, row in sorted_scores.iterrows():
                        f.write(f"{row['PDB-ID']}: {row['FE-score']:.4f}\n")
                colors = [cmap(i / len(fragment_df)) for i in range(len(fragment_df))]
                plt.figure(figsize=(10, 6))
                plt.bar(fragment_df["pdb_short"], fragment_df["FE-score"], color=colors, width=0.4, alpha=0.8, edgecolor='black')
                plt.xlabel('Fragment', fontsize=14, fontweight='bold')
                plt.ylabel('FE-score', fontsize=14, fontweight='bold')
                plt.title(f'FE-score of {fragment}', fontsize=16, fontweight='bold')
                plt.xticks(rotation=90, fontsize=12, fontweight='bold')
                plt.yticks(fontsize=12, fontweight='bold')
                plt.tight_layout()
                plot_path = os.path.join(fragment_dir, f"{fragment}_bar_chart.png")
                plt.savefig(plot_path, dpi=300)
                plt.close()
                top_pdbs = fragment_df.nlargest(5, 'FE-score')['Matched_PDB'].values
                pdb_files = glob.glob(os.path.join(pdb_folder, '**/*.pdb'), recursive=True)
                for pdb in top_pdbs:
                    pdb_no_hfe = f"{pdb}.pdb"
                    pdb_with_hfe = f"{pdb}_H_FE.pdb"
                    matching_pdbs = [file for file in pdb_files if os.path.basename(file) == pdb_no_hfe or os.path.basename(file) == pdb_with_hfe]
                    if pdb_no_hfe in [os.path.basename(f) for f in matching_pdbs]:
                        for pdb_file in matching_pdbs:
                            if os.path.basename(pdb_file) == pdb_no_hfe:
                                shutil.copy(pdb_file, fragment_dir)
                    else:
                        for pdb_file in matching_pdbs:
                            shutil.copy(pdb_file, fragment_dir)
        if self.FE_score:
            parent_folder1 = parent_folder
            fe_score_folder = os.path.join(parent_folder1, "library_analysis", "FE_score")
            merged_fe_score_file = os.path.join(fe_score_folder, "merged_FE_score.txt")
            merge_fe_score_files(fe_score_folder, merged_fe_score_file)
            merge_fmo_phore_data(parent_folder1, merged_fe_score_file)
            FE_score_charts = os.path.join(parent_folder1, "library_analysis", "FE_score_charts")
            pdb_folder = parent_folder1
            generate_fragment_barcharts(merged_fe_score_file, FE_score_charts, pdb_folder)
            ###############################################################################                
            def select_top_fragments(parent_folder, FE_score_charts):
                suggested_frags_dir = os.path.join(parent_folder, "library_analysis", "suggested_frags")
                if not os.path.exists(suggested_frags_dir):
                    os.makedirs(suggested_frags_dir)
                fragment_folders = [f for f in os.listdir(FE_score_charts) if os.path.isdir(os.path.join(FE_score_charts, f))]
                for fragment_folder in fragment_folders:
                    fe_score_file = os.path.join(FE_score_charts, fragment_folder, "FE_score_values.txt")
                    if not os.path.exists(fe_score_file):
                        print(f"Skipping {fragment_folder}, FE_score_values.txt not found.")
                        continue
                    top_pdb_fragment = None
                    top_score = float('-inf')
                    with open(fe_score_file, "r") as f:
                        for line in f:
                            parts = line.strip().split(": ")
                            if len(parts) == 2:
                                pdb_name, score = parts
                                if "frag" in pdb_name and float(score) > top_score:
                                    top_pdb_fragment = pdb_name  
                                    top_score = float(score)
                    if top_pdb_fragment:
                        base_pattern = re.match(r"^(.*?_frag\d+)", top_pdb_fragment)
                        if base_pattern:
                            base_name = base_pattern.group(1)  
                            pdb_regex = re.compile(rf"^{re.escape(base_name)}.*\.pdb$", re.IGNORECASE)
                            all_pdbs = glob.glob(os.path.join(parent_folder, "**", "*.pdb"), recursive=True)
                            matching_pdbs = [pdb for pdb in all_pdbs if pdb_regex.search(os.path.basename(pdb))]
                            if matching_pdbs:
                                selected_pdb = matching_pdbs[0]  
                                dest_path = os.path.join(suggested_frags_dir, os.path.basename(selected_pdb))
                                if os.path.abspath(selected_pdb) != os.path.abspath(dest_path):
                                    shutil.copy(selected_pdb, dest_path)
                                else:
                                    print(f"Skipping {selected_pdb}, already in suggested_frags.")
            select_top_fragments(parent_folder1, FE_score_charts)
        ###############################################################################    
        ...
        def main(parent_dir):
            ...
            for file_name in os.listdir(library_analysis_path):
                if file_name.endswith('.log') or file_name.endswith('_H_ligand_heatmap.csv') or file_name.endswith('_H_FE_ligand_heatmap.csv'):
...
if __name__ == "__main__":
    ...
    pdb_processor = LibAnalysis(args.pdb_file, args.Binding_Energy, args.FE_score)
