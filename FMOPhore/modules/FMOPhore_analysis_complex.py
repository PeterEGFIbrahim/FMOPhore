...
class ComplexAnalysis:
    def __init__(self, pdb_file, same_target=False, Binding_Energy=False):
    ...
    def com_analyze(self):
        ...
        output_file=f'{self.pdb_file[:-4]}_ligand_heatmap.csv'
        ...
        df_map_total
        df_map_total.to_csv (f'{self.pdb_file[:-4]}_export_dataframe.csv', header=True)
        df_map_total
        ...
        plt.savefig(f'{self.pdb_file[:-4]}_DFTB_heatmap_lig.png')
        plt.close()
        ###################################################################################################
        cols=df.columns.drop(['Fragment','DL'])
        df[cols] = df[cols].apply(pd.to_numeric, errors='coerce')
        ...
        plt.ylabel("kcal/mol")
        plt.tight_layout(pad=2)
        plt.savefig((os.path.splitext(FMO_file[0])[0]) + f"_total.png")
        plt.close()
        df.to_csv(f'{self.pdb_file[:-4]}_PIEDA_values.txt', sep='\t',float_format='%.3f')
        # ###################################################################################################
        # Transpose the data
        # ###################################################################################################
        df_subset = df.iloc[:, [0] + list(range(9, 15))].reset_index(drop=True)
        df_subset.to_csv(f'{self.pdb_file[:-4]}_Binding_site_energies.txt', sep='\t', float_format='%.3f', index=False)
        ###################################################################################################
        ###################################################################################################
        ...
        Edisp_list.append(df['Edisp'].sum())
        Gsol_list.append(df['Gsol'].sum())
        with open(f"{self.pdb_file[:-4]}_totals.csv","w") as f:
        ...
        with open(f"{self.pdb_file[:-4]}_total_average.txt", "a") as outfile:
        ...
        if Total_EunDsol2 is not None and one_body is not None and two_body is not None and one_body_gsol is not None and two_body_gsol is not None and two_body_interaction is not None:
            ...
            with open(f'{self.pdb_file[:-4]}_complex_energy.txt', 'w') as f:
                f.write(f'complex_energy: {complex_energy} kcal/mol')
        else:
            print('Error: Unable to calculate total energy.')
        ###################################################################################################
        with open(f'{self.pdb_file[:-4]}_totals.csv', 'r') as infile:
            reader = csv.reader(infile)
            next(reader)  
            row = next(reader)  
            delta_E = float(row[1])  
        with open(f'{self.pdb_file[:-4]}_FMO.txt', 'w') as outfile:
        ...
        if self.Binding_Energy:
            with open(f'{self.pdb_file[:-4]}_complex_energy.txt', 'r') as complex_file, \
                 open(f'{self.pdb_file[:-4]}_protein_energy.txt', 'r') as protein_file, \
                 open(f'{self.pdb_file[:-4]}_ligand_energy.txt', 'r') as ligand_file:
            ...
            with open(f'{self.pdb_file[:-4]}_Binding_Energy.txt', 'w') as f:
                f.write("Binding Energy (ΔE) = (E_Complex) − (E_Protein) − (E_Ligand) = {:.2f} kcal/mol\n".format(binding_energy))
            ###############################################################################################
            BE = []
            with open(f'{self.pdb_file[:-4]}_Binding_Energy.txt', 'r') as infile:
                for line in infile:
            ...
            delta_G = []
            logP = []
            with open(f'{self.pdb_file[:-4]}_Desc.txt', 'r') as infile:
                for line in infile:
            ...
            with open(f'{self.pdb_file[:-4]}_Affinity.txt', 'w') as outfile:
                ...
                outfile.write("- = -------- = -\n")
            ###############################################################################################
            with open(f'{self.pdb_file[:-4]}_Energies.txt', 'w') as outfile:
                for filename in [f'{self.pdb_file[:-4]}_Binding_Energy.txt', f'{self.pdb_file[:-4]}_FMO.txt', f'{self.pdb_file[:-4]}_DA_FMO.txt', f'{self.pdb_file[:-4]}_Affinity.txt']:
            ...
            with open(f'{self.pdb_file[:-4]}_Energies.txt', 'w') as outfile:
                for filename in [f'{self.pdb_file[:-4]}_FMO.txt']:
                    with open(filename, 'r') as infile:
                        outfile.write(infile.read())
                    outfile.write('- = -------- = -\n')
        ###################################################################################################
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lib', '--same_target', action='store_true', default=None, help=argparse.SUPPRESS)
    parser.add_argument('-BE', '--Binding_Energy', action='store_true', default=None, help=argparse.SUPPRESS)
    args = parser.parse_args()
    pdb_processor = ComplexAnalysis(args.pdb_file, args.same_target, args.Binding_Energy)
